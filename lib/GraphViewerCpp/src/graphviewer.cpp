...
GraphViewer::Edge& GraphViewer::addEdge(id_t id, Node &u, Node &v, Edge::EdgeType edge_type, id_t reverse_id){
    lock_guard<mutex> lock(graphMutex);
    if(edges.count(id) || (edge_type == Edge::EdgeType::DIRECTED && reverse_id != 0))
        throw invalid_argument("An edge with that ID already exists");
    if(edge_type == Edge::EdgeType::UNDIRECTED && edges.count(reverse_id))
        throw invalid_argument("An edge with that ID already exists");
    Edge &ret = *(edges[id] = new Edge(id, u, v, edge_type));
    if (edge_type == Edge::EdgeType::UNDIRECTED)
        edges[reverse_id] = edges[id];
